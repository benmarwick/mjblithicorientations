---
title: Additional statistical and graphical methods for analyzing site formation processes
  using artifact orientations
csl: plos-one.csl
date: '`r Sys.Date()`'
output:
  pdf_document: 
...
The 3D orientation of clasts within a deposit are known to be informative on processes that formed that deposit.  In archaeological sites, a portion of the clasts in the deposit are introduced by non-geological processes, meaning typically through human behaviors but also in some instances from other biological agents such as carnivores.  The find location of these non-geogenic clasts are typically systematically recorded in archaeological excavations, and with the introduction of total stations for precise piece proveniencing [@dibble_measurement_1987] it became possible to record an additional point for elongated finds to quickly and precisely capture their orientation in the normal course of documenting the excavation [@kluskens_archaeological_1995; @mcpherron_artifact_2005].  Thus sometimes relatively substantial and informative data sets for the study of post-depositional formation processes can be amassed with little additional effort.  The statistical and graphical techniques for analyzing these data are well published [@benn_fabric_1994; @lenoble_fabric_2004; @mcpherron_artifact_2005], and there is a growing set of actualistic and archaeological comparative data to help with the interpretation of the documented patterns [e.g. @bertran_fabric_1995; @kluskens_archaeological_1995; @dibble_testing_1997; @enloe_geological_2006;  @figueiredo_artifact_2007; @benito-calvo_sedimentological_2009; @chase_processes_2009; @todisco_paleoeskimo_2009; @bernatchez_taphonomic_2010; @bertran_impact_2010; @benito-calvo_trampling_2011; @benito-calvo_analysis_2011; @sitzia_paleoenvironment_2012; @de_la_torre_application_2013; @dominguez-rodrigo_testing_2013; @stratford_biofabric_2013; @hovers_islands_2014; @zwyns_open-air_2014; @byers_flake_2015; @pei_middle_2015; @lotter_geoarchaeological_2016; @wright_approaches_2017].

This paper advances this area of research in presenting several new methodologies for analyzing orientation data.  These methods are designed to address several shortcomings in current methodologies.  First, one difficulty in orientations analysis is in knowing how large a sample is needed to have a reliable or, more importantly, informative measure. Neudorf et al. [-@neudorf_comparisons_2015] use eigenstatistics (see below) to argue that after approximately 35 measurements variability is minimized enough to achieve robust results.  Lenoble and Bertran [-@lenoble_fabric_2004] use three data sets with unknown orientation characteristics to show that the Vector Magnitude (L) statistic (a measure of the strength of the dominant bearing in an assemblage) stabilizes after about 40 or 50 artifacts, and they argue that with this size of sample highly linear assemblages can be distinguished from other kinds of assemblages.  In other words, the required sample size is in part a function of the distribution of orientations in the assemblage.  This point is emphasized by Ringrose and Benn [-@ringrose_confidence_1997;@benn_random_2001] who also argue that 50 observations are required to reach a conclusion about the orientations. To show this and to provide a solution to the sample size problem, they propose a method for calculating confidence intervals for Benn diagrams [@benn_fabric_1994].  While Benn diagrams have proven particularly useful for representing and comparing three dimensional orientations [@lenoble_fabric_2004; @mcpherron_artifact_2005], in a Benn diagram assemblages are represented by a single point with no indication of the associated uncertainty [@ringrose_confidence_1997].  Thus to address these issues here, rather than arriving at specific recommendations for sample sizes, a method for calculating 95% confidence intervals on Benn diagrams is presented based on the work of Ringrose and Benn [@ringrose_alternative_1996; @ringrose_confidence_1997; @benn_random_2001] and on what has been proposed by Weaver and Steele [-@steele_modified_2002; @weaver_cross-platform_2011] for assessing mortality profiles in faunal assemblages also ternary diagrams. With confidence intervals, the interaction between the underlying orientation tendencies and sample size are exposed.  These results confirm that the size of the confidence interval for a given sample size depends very much on the structure of the data or, in other words, the type of post-depositional disturbance that may have occurred.  Confidence intervals also allow for a better assessment of whether and what kinds of post-depositional disturbances may have affected the deposit by showing potential overlap between the uncertainty in the observed data and the convex hulls of sets of points known already to represent various kinds of processes [@lenoble_fabric_2004].

Second, another difficulty in orientations analysis is in knowing just how similar or different assemblages are in their three dimensional orientation structure (i.e. after having computed eigenvalues and plotting these on a Benn diagram)[@ringrose_confidence_1997].  Though there are tests for comparing bearing angles (e.g. Rayleigh test) or plunge angles (e.g. unpaired t-test), to my knowledge there exists no recommended test for comparing the two together.  The above mentioned confidence intervals on the Benn diagrams do help give some indication of the degree to which assemblages may differ [@benn_random_2001; @larsen_fabric_2003; @neudorf_comparisons_2015], and here an additional option of applying a permutations analysis to the Euclidean distance between the Benn indices (elongation and isotropy) is presented to address this problem.  
...
lf = readRDS(here::here('McPherron/LF_orientations.RDS'))
...
sample = lf %>%
  group_by(LEVEL, CODE) %>%
  summarise(n = n()) %>%
  cast(LEVEL~CODE, value = 'n') %>%
  arrange(desc(LEVEL))

obj_length = lf %>%
  group_by(LEVEL) %>%
  summarise(obj_length = round(median(obj_length),3)) %>%
  arrange(desc(LEVEL)) 

lithics = lf %>%
  group_by(LEVEL) %>%
  summarise(n_elongation = sum(!is.na(elongation)),
            mean_elongation = round(mean(elongation, na.rm = TRUE),2),
            sd_elongation = round(sd(elongation, na.rm = TRUE),2) ) %>%
  arrange(desc(LEVEL)) 

sample = cbind(sample, obj_length[,-1], lithics[,-1])
colnames(sample) = c('Layer','Bones','Lithics','Length (Median)','Elongation (n)','Elongation (Mean)','Elongation (SD)')

rownames(sample) = sample$Layer

sample[sample=='NaN'] = ''
sample[sample==0] = ''

print(kable(sample[,-1],
             caption = 'La Ferrassie two-shot sample characteristics by layer. Elongation mean and standard deviation are calculated on a sub-sample of lithics with length and width measurements made with calipers (see text for how these measurements were made).  Median length is calculated based on the two total station coordinates for the object.'),
             comment = FALSE)
...
```{r figure1, fig.height = 8.5, fig.width = 6.5}
...
# Load comparative data from Lenoble and Bertran 2004 and assign colors
...
rad = function(x) return(x / 360 * 2 * pi)

source(here::here("McPherron/orientations.R"))

p = .95

# Create a dataset of 1000 points with uniform bearing angles 0-360
# and plunge angles -10 to 10 uniform
planar_points = data.frame(bearing = runif(n = 1000, min = 0, max = 360),
                           plunge = runif(n = 1000, min = -20, max = 20),
                           X1 = 0, Y1 = 0, Z1 = 0)
planar_points$Z2 = tan(rad(planar_points$plunge))
d = sqrt(1^2 - planar_points$Z2 ^ 2)
planar_points$X2 = sin(rad(planar_points$bearing)) * d
planar_points$Y2 = cos(rad(planar_points$bearing)) * d

# Create a dataset of 1000 points with uniform bearing angles 160-200
# and uniform plunge angles -10 to 10
linear_points = data.frame(bearing = runif(n = 1000, min = 160, max = 200),
                           plunge = runif(n = 1000, min = -20, max = 20)
                           , X1 = 0, Y1 = 0, Z1 = 0)
linear_points$Z2 = tan(rad(linear_points$plunge))
d = sqrt(1^2 - planar_points$Z2 ^ 2)
linear_points$X2 = sin(rad(linear_points$bearing)) * d
linear_points$Y2 = cos(rad(linear_points$bearing)) * d

if (tiff_figures) tiff('figure_01.tif', res = 300, width = 6.5, height = 6.5, units = 'in')

layout(matrix(c(1:12), nrow = 4, ncol = 3))

for (n in seq(50, 250, 100)) {
  resampling_contours = list()
  for (proportion in c(0, 0.2, 0.5, 0.8)) {
    sample_size = floor((1 - proportion) * n)
    xyz = planar_points[sample(x = 1000, size = sample_size, replace = TRUE),]
    if (proportion != 0) 
      xyz = rbind(xyz, linear_points[sample(x = 1000, size = n - sample_size, replace = TRUE),])
    xyz$sample_name = proportion
    benn_diagram(benn(xyz, min_sample = 30), cex = .7,
               main = ifelse(proportion == 0,
                             paste('Sample size ',n,'\nwith ',round(proportion*100,0),'% linear', sep = ''),
                             paste(round(proportion*100,0),'% linear', sep = '')),
               labels = ifelse(n == 50 & proportion == 0, 'outside', 'none'),
               dimnames_position = ifelse(n == 50 & proportion == 0, 'corner', 'none'),
               grid_labels = ifelse(n == 50 & proportion == 0, TRUE, FALSE))
    ...
    benn_diagram(lenoble_and_bertran, 
         ...
         legend_names = switch((n == 50 & proportion == 0) + 1,
                               NULL, levels(factor(lenoble_and_bertran$Type))),
         legend_colors = formation_process_colors)

    for (contour in benn_resampling(xyz, resampling = resampling, p = p))
      lines(benn_coords(cbind(elongation = contour$x, isotropy = contour$y)))

    if (proportion == 0) dataset = xyz else dataset = rbind(dataset, xyz)
  }
}
...
**Fig 1.  Confidence intervals on simulated data showing the effect of sample size with varying proportions of planar and linear data.  The black dot in each case shows the actual calculated value for that assemblage.  Each column of figures has the same sample size.  Each row of figures has the same proportional mix of planar and linear samples.  E1, E2 and E3 are the first, second and third eigenvalues respectively.  Comparative deposition data from @lenoble_fabric_2004.**

```{r figure2, fig.height = 6.5, fig.width = 6.5}

benn_lf = round(benn(xyz = lf, level = lf$LEVEL, min_sample = 30), 3)
...
for (levelname in rev(unique(lf$LEVEL))) {

	xyz_level = lf %>% filter(LEVEL==levelname)
	
	benn_diagram(cbind(benn_lf[levelname,"EL"],benn_lf[levelname,"IS"]),
	             main = paste("Layer",levelname),
	             cex = .7,
               labels = ifelse(levelname == rev(unique(lf$LEVEL))[1], 'outside', 'none'),
               dimnames_position = ifelse(levelname == rev(unique(lf$LEVEL))[1], 'corner', 'none'),
               grid_labels = ifelse(levelname == rev(unique(lf$LEVEL))[1], TRUE, FALSE))

  d3dd(lenoble_and_bertran, 
           drawhull = TRUE,
           new_page = FALSE,
           plot_points = FALSE, cex = .7,
           col = lenoble_and_bertran_colors,
           legend_names = switch((levelname == rev(unique(lf$LEVEL))[1]) + 1,
                                 NULL, levels(factor(lenoble_and_bertran$Type))),
           legend_colors = formation_process_colors)
...
**Fig 2.  Benn diagrams for each La Ferrassie layers.  The dot indicates the calculated values for that layer. The confidence intervals represent a probability of `r p` based on `r resampling` times resampling.**

```{r figure 3, fig.height = 6.5, fig.width = 6.5}

if (tiff_figures) tiff('figure_03.tif', res = 300, width = 6.5, height = 6.5, units = 'in')

benn_diagram(cbind(benn_lf[,"EL"], benn_lf[,"IS"]),
             id = rownames(benn_lf),
             main = "",
             cex = 1.2,
             labels = "outside")
...
permutations = benn_permutations_by_level(lf, level = lf$LEVEL, resampling = resampling, p = p)
...
**Fig 3.  Benn diagram for all La Ferrassie layers.  Connecting lines indicate layers that do not differ statistically at the p=`r p` level based on a permutations test with `r resampling` times resampling (see Table 2).**

```{r table2, results = 'asis'}
print(xtable(permutations$results,
             caption = paste('Probabilities resulting from permutation test results of layer pair comparison.  Results based on',resampling,' times resampling.  Probabilities >',p,'are plotted with connecting lines in Fig 3.')),
             comment = FALSE)
```

```{r figure 4, fig.height = 4.25, fig.width = 6.5}

lf_background_image = "The Whole Site (small).tif"

lf_limits = c(101, 108, 105, 112)

if (tiff_figures) {
  fig_no = 3
  odd_even = 1
  for (levelname in unique(lf$LEVEL)) {
    odd_even = odd_even + 1
    if (odd_even == 2) {
      fig_no = fig_no + 1
      if (fig_no != 8) {
        tiff(paste('figure_0',fig_no,'.tif',sep=''),
             res = 300, width = 6.5, height = 9, units = 'in')
        layout(m = matrix(c(1,1,1,5,5,5,1,1,1,5,5,5,2,3,4,6,7,8),
                          nrow = 6, ncol = 3))
      } else {
        tiff(paste('figure_0',fig_no,'.tif',sep=''),
             res = 300, width = 6.5, height = 4.5, units = 'in')
        layout(m = matrix(c(1,1,1,1,1,1,2,3,4), nrow = 3, ncol = 3))
      }        
      odd_even = 0 }
    lf_stat = orientations(lf %>% filter(LEVEL==levelname), overlay = NULL,
                           spatial_benn = TRUE, limits = lf_limits,
                           background_image = lf_background_image,
                           main = paste("Layer", levelname))
    if (odd_even == 1) graphics.off() }
  
} else {
  layout(m = matrix(c(1,1,1,1,1,1,2,3,4), nrow = 3, ncol = 3))
  lf_stat = orientations(lf, level = lf$LEVEL, overlay = NULL,
                         spatial_benn = TRUE, limits = lf_limits,
                         background_image = lf_background_image,
                         main = paste("Layer", unique(lf$LEVEL)))  }

```
  
**Figs 4-8.  Artifact orientations for La Ferrassie.  The plan view shows all two-shot bone and stone artifacts. The underlying image is a georeference orthophoto extracted from a structure from motion model.  With the exception of Layer 1, the plan view points are color-coded using Benn statistics computed on the 40 closest artifacts.  The color key is shown in the Benn diagram where these same points are also plotted as black dots.  Each figure also includes a Schmidt lower hemisphere plot summarizing bearing and plunge angles with a superimposed Rose diagram summarizing only the bearing angle distributions [@mcpherron_artifact_2005].  Below this is a circular histogram of plunge angles.  Though the sample size in Layer 1 is not large enough for spatial color coding, the layer is included here for completeness of the presentation of La Ferrassie artifact orientations.**

```{r figure 5, fig.height = 6.5, fig.width = 6.5}

if (tiff_figures) tiff('Figure_09.tif', res=300, width = 6.5, height = 6.5, units = 'in')

layout(m = matrix(c(1,3,2,4), nrow = 2, ncol = 2))

for (nearest in seq(10, 70, by = 20)) {
	lf %>% 
    filter(LEVEL=='2') %>% {
    color_codes = spatial_benn(., nearest = nearest)
    plot_2shot(., main = paste('Sampling Window of', nearest),
               limits = lf_limits,
...
__Supporting Information__

**S1 File.  R Markdown file used to create this document (mcpherron_2017.rmd).**

**S2 File.  Le Ferrassie artifact orientation data set (LF_orientations.RDS).**

**S3 File.  Lenoble and Bertran (2004) comparative data set (Lenoble_and_Bertran_2004.RDS).**
