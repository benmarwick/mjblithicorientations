...
circular_statistics = function(data, level = "All observations", min_sample = 50) {
  ...
  library(CircStats)
    ...
    xyz = TRUE
	  results = matrix(NA, nrow=length(levels(level)), ncol=12,
	                   dimnames = list(levels(level),c("N","Length_Mean","Length_SD","Bearing_L","Bearing_Mean",
	                                                   "Bearing_Var","Bearing_P","Plunge_L","Plunge_Mean","Plunge_Var","Plunge_P", "Plunge_K"))) }
	else {
	  xyz = FALSE
	  results = matrix(NA, nrow=length(levels(level)), ncol=10,
	                   dimnames = list(levels(level),c("N","Bearing_L","Bearing_Mean","Bearing_Var","Bearing_P",
	                                                   "Plunge_L","Plunge_Mean","Plunge_Var","Plunge_P", "Plunge_K"))) }
	...
	for (a_level in levels(level)) {
        ...
        vector_length_sd = NA
			...
			Plunge_P = round(ks.test(pb$plunge,punif,min=0,max=90)$p.value, 20)
			Plunge_K = round(ks.test(pb$plunge,punif,min=0,max=90)$statistic, 4)
...
#---------------------------------------------------------------------------
# BM's functions, not in McPherron's paper or repo
#---------------------------------------------------------------------------

# this function will draw a Benn diagram that plots multiple analytical units on one triangle
# and shows the segments connecting units that do not significantly differ
benn_diagram_for_all_units <- function(xyz,
                                       p = 0.05,
                                       resampling = 10000,
                                       min_sample = 3){

  benn_rounded = round(benn(xyz = xyz,
                            level = xyz$phase_groups,
                            min_sample = min_sample), 3)

  benn_diagram(cbind(benn_rounded[,"EL"], benn_rounded[,"IS"]),
               id = rownames(benn_rounded),
               main = "",
               cex = 1.2,
               labels = "outside")

  # Connecting lines indicate layers that do not differ statistically at the
  # p = 0.05 level based on a permutations test with many (10000) times resampling

  permutations_fb =
    benn_permutations_by_level(xyz,
                               level = as.character(xyz$phase_groups),
                               resampling = resampling,
                               min_sample = min_sample,
                               p = p)

  segments(permutations_fb$segments[,1],permutations_fb$segments[,2],
           permutations_fb$segments[,3],permutations_fb$segments[,4])
}
...
# Here is a function we will apply to make one composite figure for each analytical unit of artefacts

make_benn_rose_plots_panel <- function(xyz_level,
                                       min_sample = 10,
                                       p = 0.95, # p value for resampling - McPherron 2018
  ...
  # Benn diagram on the left...

  # Summary stats for isotropic ratio and elongation ratio for each layer
  benn_summary = round(benn(xyz = xyz_level,
                            level = xyz_level$phase_groups, # note that 'phase_groups' is specific to our data
                            min_sample = min_sample), 3)

  benn_diagram(cbind(benn_summary[,"EL"],benn_summary[,"IS"]),
               main = unique(xyz_level$phase_groups),
               cex = .7,
               labels = 'outside',
               dimnames_position =  'corner',
               grid_labels = TRUE)
  ...
  benn_diagram(lenoble_and_bertran,
               ...
               legend_names = levels(factor(lenoble_and_bertran$Type)),
               legend_colors = formation_process_colors)

  # compute resampling contours
  resampling_contours = benn_resampling(xyz_level,
                                        resampling = resampling,
                                        p = p,
                                        min_sample = min_sample)

  # draw them on the Benn diagram
  for (contour in resampling_contours)
    lines(benn_coords(cbind(elongation = contour$x,
                            isotropy = contour$y)))

  # Rose diagram of bearing in the middle...

  rose_diagram(plunge_and_bearing(xyz_level)$bearing, main = "Bearing", cex = .8)
  schmidt_diagram(angles = plunge_and_bearing(xyz_level), redraw=FALSE)

  # Rose diagram of plunge on the right

  rose_diagram_plunge(plunge_and_bearing(xyz_level)$plunge, main = "Plunge", cex = .8)
...
kuiper_and_rayleigh_and_ks_tests <- function(xyz,
                                             min_sample = 5){
  bearing_angle = plunge_and_bearing(xyz)$bearing
  ...
  k_test <- capture.output(kuiper.test(bearing_angle)) # capture output so we can tidy later
  r_test <- rayleigh.test(bearing_angle)

  # Kolmogorov-Smirnov test for plunge

  # Circle stats on bearing
  # Bearing_L           = Rayleigh Test
  # Bearing_Mean        = mean
  # Bearing_Var         = circular Dispersion
  # Bearing_P           = Rayleigh Test p-value
  # Plunge_L            = Rayleigh Test
  # Plunge_Mean         = mean
  # Plunge_Var          = standard deviation
  ks_test_on_plunge <-
    as_tibble(circular_statistics(xyz, min_sample = min_sample)) %>%
    dplyr::select(
      N,
      `KS statistic` = Plunge_K,
      `KS p-value` =   Plunge_P)

  # tidy the outputs into tibble

  three_tests_tbl <- tribble(~Kuiper, ~`Kuiper p-value`, ~Rayleigh, ~`Rayleigh p-value`,
                             parse_number(k_test[4]),
                             str_squish(k_test[5]),
                             r_test$statistic,
                             as.character(round(r_test$p.value, 3))) %>%
    bind_cols(ks_test_on_plunge)

  return(three_tests_tbl)

}

# from https://github.com/benmarwick/mjbnaturepaper/blob/master/R/functions.R#L1471
# because mjbnaturepaper has some tricky dependencies
